 ...
 * (текст и голос). Ассистент отвечает с учетом контекста визарда (/api/ai/chat).
 */
  ...
  async function applyDiagnosticsFromChat(text: string, role: string) {
    try {
      ...
      if (filled > 0 || notes) {
        pushChat(
          "ai",
          `📝 Отметил ваши ответы в анкете справа${filled ? ` (пунктов: ${filled})` : ""}. Можно сразу нажать «Сформировать предложение» или добавить еще.`
        );
      }
    } catch {
    ...
    try {
      ...
      pushChat(
        ...
        e instanceof Error && e.message.length < 160
          ? e.message
          : "Не удалось получить ответ. Попробуйте еще раз или продолжите заполнение формы."
      );
    } finally {
      setSending(false);
    }
