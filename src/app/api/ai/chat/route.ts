...
const STEP_NAMES: Record<number, string> = {
  ...
  3: "смотрит персональное предложение и расчет",
  ...
  try {
    ...
    const ctxLines = [
      ...
      c.step === 1 && (c.missingCompanyFields?.length ?? 0) > 0
        ? `- В форме шага 1 еще не заполнено: ${c.missingCompanyFields!.join(", ")}. Спроси об этом (одним коротким вопросом за раз) — ответы клиента система сама подставит в форму.`
        : "",
      c.step === 2
        ? `- Анкета: отвечено ${c.answeredCount ?? 0} из ${c.totalQuestions ?? "?"} вопросов.` +
          ((c.pendingQuestions?.length ?? 0) > 0
            ? ` Еще не отвечены: ${c.pendingQuestions!.map((q) => `«${q}»`).join("; ")}.`
      ...
      c.step === 2
        ? "- Ты ВЕДЕШЬ диагностику: задай ОДИН следующий неотвеченный вопрос анкеты своими словами, с 3–5 примерами вариантов через запятую. Ответы клиента система сама отмечает в анкете справа."
        : "",
    ...
    // берем последние 12 реплик, чтобы не раздувать промпт
    ...
    return NextResponse.json(
      {
        error:
          "Не удалось получить ответ ассистента. Попробуйте еще раз или продолжите заполнение формы.",
      },
      { status: 502 }
    );
  }
