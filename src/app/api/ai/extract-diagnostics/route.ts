 ...
 * сам подставляет дословные строки вариантов. Никогда не бросает — при сбое отдает пусто.
 */
...
ПРИМЕР: [q01] 1) Отчеты 2) Письма → "пишем письма" → {"matches":[{"qid":"q01","optionNumbers":[2],"other":""}],"notes":""}
...
Верни ТОЛЬКО JSON {"matches":[{"qid":"код","optionNumbers":[],"other":""}],"notes":""}`;
    ...
    const extracted = await chatJson(
      ...
      (d) => ExtractSchema.parse(d),
      {
        ...
        temperature: 0.2,
        maxTokens: 800, // список совпадений по номерам вариантов
      }
    );
