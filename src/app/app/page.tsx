  ...
  return (
    <div className="sr-only">
      ...
      <p>
        ...
        AI-диагностика ВайбМайнд подберет модули корпоративного курса по
...
export default function WizardPage() {
   ...
   * вперед — только если данные шага готовы (есть предложение / бронь).
   */
  function canGoTo(n: number): boolean {
    if (n === step) return false;
    if (n < step) return true; // назад всегда можно
    if (n === 3 || n === 4) return !!proposal; // предложение и возражения — после расчета
    if (n === 5) return !!proposal; // выбор времени — после предложения
    return false; // шаг 2 вперед — только через кнопку «Начать AI-диагностику»
  }
